/*
 * Original source:
 * https://github.com/doitsujin/dxbc-spirv/blob/main/util/util_md5.cpp
 * https://github.com/doitsujin/dxbc-spirv/blob/main/dxbc/dxbc_container.cpp
...
Digest Hasher::getDigest() {
  ...
  for (uint32_t i = 0u; i < m_state.size(); i++) {
    ...
    for (uint32_t j = 0u; j < sizeof(dw); j++)
      digest.data.at(sizeof(dw) * i + j) = bit::bextract(dw, 8u * j, 8u);
  }
...
void Hasher::processBlock(const unsigned char *data) {
  ...
  auto finalizeIteration = [&state, data, s = s_shifts.data(),
                            k = s_constants.data()](uint32_t i, uint32_t f,
                                                    uint32_t g) {
...
void Hasher::padBlock() {
  ...
  for (uint32_t i = 0u; i < finalPadding.size(); i++)
    finalPadding.at(i) = uint8_t(bit::bextract(bitCount, 8u * i, 8u));
...
Digest hashDxbcBinary(const void *data, size_t size) {
  ...
  constexpr size_t offset = 20; // A fixed value
  ...
  if (remainder >= 56u) {
    ...
    hasher.update(&aNum, sizeof(aNum));
    hasher.update(s_padding.data() + sizeof(aNum),
                  s_padding.size() - sizeof(aNum) - sizeof(bNum));
    hasher.update(&bNum, sizeof(bNum));
  } else {
    /* Append bit count */
    hasher.update(&aNum, sizeof(aNum));
    ...
    hasher.update(s_padding.data(), paddingSize - sizeof(aNum) - sizeof(bNum));
